...
public class Config implements Tools.IPublic {
	...
	public boolean setLine(String line) {
		...
		line = Tools.String.getFirstLine(line);
		line = Tools.String.clearLRSpace(line);
	...
	public boolean setField(String field) {
		...
		field = Tools.String.clearLRSpace(field);
		this.field = field;
		return true;
	}
	public boolean setValue(String value) {
		...
		value = Tools.String.clearLRSpace(value);
		this.value = value;
		if(this.value.length() == 0) {
			this.items = new String[0];
			return true;
		}
		this.items = Tools.String.split(value, '|');
		return true;
	}
	...
	public boolean fetchFirstBoolean() {
		...
		try {
			...
			ok = true;
			setValue(Tools.String.link(items, "|", 1, items.length-1));
			return res;
		} catch(Exception e) {
			ok = false;
			return res;
	...
	public boolean fetchLastBoolean() {
		...
		try {
			...
			ok = true;
			setValue(Tools.String.link(items, "|", 0, items.length-2));
			return res;
		} catch(Exception e) {
			ok = false;
			return res;
