...
public class Manager implements com.FileManagerX.Interfaces.IDepotManager{
	...
	public boolean renameFile(String sourUrl, String targetName) {
		...
		try {
			...
			String destUrl = com.FileManagerX.Tools.Url.setName(sourUrl, targetName);
			java.io.File dest = new java.io.File(destUrl);
			boolean ok = sour.renameTo(dest);
			if(ok && dest.isDirectory()) {
				com.FileManagerX.Tools.Update.fixSingleFolderUrl(sour.getAbsolutePath(), dest.getAbsolutePath());
			}
			if(ok && dest.isFile()) {
				com.FileManagerX.Tools.Update.fixSingleFileUrl(sour.getAbsolutePath(), dest.getAbsolutePath());
			}
			return ok;
		} catch(Exception e) {
			com.FileManagerX.BasicEnums.ErrorType.COMMON_FILE_OPERATE_FAILED.register(e.toString());
	...
	public boolean renameFileWithoutExtension(String sourUrl, String targetNameWithoutExtension) {
		...
		try {
			java.io.File sour = new java.io.File(sourUrl);
			String destUrl = com.FileManagerX.Tools.Url.setNameWithoutExtension(sourUrl, targetNameWithoutExtension);
			java.io.File dest = new java.io.File(destUrl);
			boolean ok = sour.renameTo(dest);
			if(ok && dest.isDirectory()) {
				com.FileManagerX.Tools.Update.fixSingleFolderUrl(sour.getAbsolutePath(), dest.getAbsolutePath());
	...
	public boolean deleteFile(String targetFile) {
		...
		try {
			...
			if(ok) {
				com.FileManagerX.Tools.Update.delSingleFile(sour.getAbsolutePath());
			}
			return ok;
		} catch(Exception e) {
			com.FileManagerX.BasicEnums.ErrorType.COMMON_FILE_OPERATE_FAILED.register(e.toString());
	...
	public boolean moveFile(String sourUrl, String destUrl) {
		...
		try {
			...
			boolean ok = sour.renameTo(dest);
			if(ok && dest.isDirectory()) {
				com.FileManagerX.Tools.Update.movSingleFolder(sour.getAbsolutePath(), dest.getAbsolutePath());
			}
			if(ok && dest.isFile()) {
				com.FileManagerX.Tools.Update.movSingleFile(sour.getAbsolutePath(), dest.getAbsolutePath());
			}
			return ok;
		} catch(Exception e) {
			com.FileManagerX.BasicEnums.ErrorType.COMMON_FILE_OPERATE_FAILED.register(e.toString());
	...
	public boolean copyFile(String sourUrl, String destUrl) {
		...
		if(ok) {
			com.FileManagerX.Tools.Update.addSingleFile(destUrl);
		}
		return ok;
	}
	...
	public boolean createFolder(String targetUrl) {
		...
		try {
			...
			boolean ok = f.mkdirs();
			if(ok) {
				com.FileManagerX.Tools.Update.addSingleFolder(f.getAbsolutePath());
			}
			return ok;
		} catch(Exception e) {
			com.FileManagerX.BasicEnums.ErrorType.COMMON_FILE_OPERATE_FAILED.register(e.toString());
	...
	public boolean createFile(String targetUrl) {
		...
		try {
			...
			if(ok) {
				com.FileManagerX.Tools.Update.addSingleFile(f.getAbsolutePath());
			}
			return ok;
		} catch(Exception e) {
			com.FileManagerX.BasicEnums.ErrorType.COMMON_FILE_OPERATE_FAILED.register(e.toString());
	...
	private boolean copyFileCore(String sourLocalUrl, String destLocalUrl) {
		try {
	        ...
	        com.FileManagerX.Tools.Update.addSingleFile(dest.getAbsolutePath());
	        return true;
		} catch(Exception e) {
			com.FileManagerX.BasicEnums.ErrorType.COMMON_FILE_OPERATE_FAILED.register(e.toString());
			return false;
		...
		try {
			...
			for(java.io.File f : subfiles) {
				if(f.isFile()) {
					...
					ok &= okThis;
					if(okThis) {
						com.FileManagerX.Tools.Update.delSingleFile(f.getAbsolutePath());
					}
				}
				if(f.isDirectory()) {
			...
			okThis = sour.delete();
			if(okThis) {
				com.FileManagerX.Tools.Update.delSingleFolder(sour.getAbsolutePath());
			}
			return ok;
		} catch(Exception e) {
			com.FileManagerX.BasicEnums.ErrorType.COMMON_FILE_OPERATE_FAILED.register(e.toString());
	...
	private boolean copyDirectoryCore(String sourLocalUrl, String destLocalUrl) {
		try {
			...
			if(!dest.exists()) {
				...
				ok &= okThis;
				if(okThis) {
					com.FileManagerX.Tools.Update.addSingleFolder(dest.getAbsolutePath());
				}
			}
	...
	private boolean isInDepot(String url) {
		...
		if(!com.FileManagerX.Tools.Url.isFileIn(url, depot.getUrl())) {
			com.FileManagerX.BasicEnums.ErrorType.COMMON_FILE_OPERATE_FAILED.register("URL Not in This Depot");
			return false;
		}
		return true;
	}
	private boolean isDepotRight() {
		...
		if(depot == null) {
			com.FileManagerX.BasicEnums.ErrorType.COMMON_FILE_OPERATE_FAILED.register("Depot is NULL");
			return false;
		}
		if(depot.getIndex() <= 0) {
			com.FileManagerX.BasicEnums.ErrorType.COMMON_FILE_OPERATE_FAILED.register("Depot Index Error");
			return false;
		}
		if(depot.getUrl() == null || depot.getUrl().length() == 0) {
			com.FileManagerX.BasicEnums.ErrorType.COMMON_FILE_OPERATE_FAILED.register("Depot Url is NULL or Empty");
			return false;
		}
